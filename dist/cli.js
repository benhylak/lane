...
import { createLane, removeLaneCmd, getLaneForSwitch, listAllLanes, getMainRepoRoot, syncLane, smartLane, findLaneByBranch, } from "./lanes.js";
import { recordLaneSwitch, getPreviousLane, loadConfig, saveConfig } from "./config.js";
...
program
    ...
    if (options.interactive) {
        const { waitUntilExit } = renderUI(React.createElement(LaneList, {
            lanes,
            onSelect: (lane) => {
                console.log(`${CD_PREFIX}${lane.path}`);
            },
        ...
        for (const lane of lanes) {
            ...
            console.log(`  ${chalk.cyan(lane.name)}${main}${current}`);
...
${MARKER_END}`;
    ...
    if (existsSync(configFile)) {
        ...
        if (existingContent.includes(MARKER)) {
            // Remove old version
            const regex = new RegExp(`${MARKER.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[\\s\\S]*?${MARKER_END.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\n?`, "g");
...
program
    ...
    const { waitUntilExit, clear } = renderUI(React.createElement(CheckoutSelector, {
        branchName,
...
program
    ...
    const { waitUntilExit } = renderUI(React.createElement(LaneManager, {
        lanes,
...
program
    ...
    const { waitUntilExit } = renderUI(React.createElement(Settings, {
